...
export function BulkApplyPlanDialog({
  planIds,
  isSubmitting,
  onClose,
  onSubmit,
}: BulkApplyPlanDialogProps) {
  ...
  return (
    <ModalShell
      ...
      titleId="bulk-apply-plan-title"
      subtitle={
        <div className="mt-0.5 flex items-center gap-1.5">
          <span className="inline-flex items-center rounded-full bg-[#245A34]/10 px-2 py-0.5 text-xs font-black text-[#245A34]">
            {planIds.length} kế hoạch
          </span>
          <span className="text-sm font-medium text-slate-400">sẽ được áp dụng</span>
        </div>
      }
      accentBar={
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                Áp dụng {planIds.length} kế hoạch
              </>
            )}
          </button>
        </>
                  ...
                  <div className="flex items-start gap-3 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3.5">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-xs font-semibold text-amber-600">
                        Chọn vườn để xác định phạm vi áp dụng.
                      </p>
                    </div>
                  </div>
                )}
