...
import type { PlantEventResponse, TreatmentStatus } from "../types";
import {
  EVENT_TYPE_LABELS,
  formatDate,
  TREATMENT_STATUS_LABELS,
} from "../components/displayUtils";
...
export function TreatmentPlanDetailPage() {
  ...
  const handleDeletePlan = async () => {
    ...
    navigate(ROUTES.DASHBOARD.TREATMENT_PLANS);
  };
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col space-y-8">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <Link
            to={ROUTES.DASHBOARD.TREATMENT_PLANS}
      ...
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="rounded-[1.75rem] border border-slate-100 bg-white p-5 shadow-sm lg:col-span-2">
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
            {[
              ["Trạng thái", TREATMENT_STATUS_LABELS[plan.status] ?? plan.status],
