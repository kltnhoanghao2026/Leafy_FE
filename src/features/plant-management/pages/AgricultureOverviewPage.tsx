...
import { EVENT_TYPE_LABELS, formatDate, TREATMENT_STATUS_LABELS } from "../components/displayUtils";
import type { PlantEventResponse, TreatmentPlanResponse } from "../types";
...
export function AgricultureOverviewPage() {
  ...
  const plansQuery = useMyTreatmentPlans();
  ...
  const activePlans = plans.filter((plan) => activeStatuses.has(plan.status));
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col space-y-8">
        ...
        <div className="mt-3 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              ["Hỏi AI", ROUTES.DASHBOARD.AI_ASSISTANT],
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        ...
        <Panel title="Kế hoạch đang xử lý" link={ROUTES.DASHBOARD.TREATMENT_PLANS}>
          <ActivePlanList
    ...
    <div className="space-y-3">
      {plans.map((plan) => (
        ...
        >
          ...
          <p className="mt-1 text-xs font-bold text-slate-500">
            {TREATMENT_STATUS_LABELS[plan.status] ?? plan.status} · {formatDate(plan.createdAt)}
          </p>
        </Link>
      ))}
    </div>
