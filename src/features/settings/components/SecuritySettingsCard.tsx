...
export function SecuritySettingsCard() {
  ...
  const updateMutation = useUpdateSecurityPreferencesMutation();
  ...
  useEffect(() => {
    if (preferences?.securitySettings) {
      setTwoFactorEnabled(preferences.securitySettings.twoFactorEnabled);
    }
  }, [preferences]);
  ...
  const handleToggle = async () => {
    ...
    try {
      await updateMutation.mutateAsync({ twoFactorEnabled: nextValue });
      setTwoFactorEnabled(nextValue);
      setMessage(t("settings.security.saved"));
    } catch (err) {
      setMutationError(
        err instanceof Error ? err.message : t("settings.security.saveError")
      );
    }
  };
    ...
    <section className="bg-[var(--app-card)] rounded-[24px] p-6 md:p-8 shadow-sm border border-slate-100 flex flex-col">
      <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
        <div className="flex items-center">
          <Shield className="w-5 h-5 text-[#245A34] mr-2" strokeWidth={2.5} />
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-slate-100/60 bg-slate-50/30 rounded-2xl p-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-bold text-slate-800 text-sm">{t("settings.security.twoFactor")}</h3>
            ...
            <div className="shrink-0 flex items-center">
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#245A34] focus:ring-offset-2 ${
                  twoFactorEnabled ? "bg-[#245A34]" : "bg-slate-200"
                } disabled:opacity-50`}
              >
                <span
