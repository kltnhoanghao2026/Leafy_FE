...
import { useSystemHealth } from "../queries/health.queries";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-slate-600 bg-white border border-slate-200 shadow-sm hover:bg-slate-50 transition-colors disabled:opacity-50 shrink-0"
        >
          <RefreshCw
      ...
      {!isLoading && !isError && banner && data && (
        <div
          className={`flex items-center gap-3 rounded-2xl border px-5 py-4 ${banner.bg}`}
        >
          <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${banner.dot}`} />
          <Activity
            className={`w-5 h-5 shrink-0 ${banner.text}`}
            strokeWidth={2}
          />
