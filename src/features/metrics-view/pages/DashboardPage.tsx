    ...
    <div className="flex-1 w-full max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div>
        <h2 className="text-[28px] font-bold text-[#111827] tracking-tight">
          Bản đồ tổng quan vườn
        </h2>
        <p className="text-[#6B7280] text-[15px] font-medium mt-1">
          Chọn một khu vực trên bản đồ để xem chi tiết thông số loT.
        </p>
      </div>

      {/* Map Section */}
      <div
        className="relative w-full h-[380px] bg-white rounded-[2rem] border-2 border-slate-100 overflow-hidden shadow-sm flex items-center justify-center p-8"
        style={{
          backgroundImage:
      ...
      >
        ...
        <div className="absolute bottom-6 right-6 z-10 flex flex-col gap-2">
          <div className="flex flex-col bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <button className="p-3 text-slate-600 hover:bg-slate-50 transition-colors border-b border-slate-100">
              <Plus className="w-5 h-5" />
            </button>
            <button className="p-3 text-slate-600 hover:bg-slate-50 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl relative z-0">
          {AREAS.map((area) => (
            <div
              key={area.id}
              onClick={() => navigate(ROUTES.DASHBOARD.ZONE_METRICS(area.id))}
            ...
            >
              ...
              <div className="absolute top-5 right-5 flex h-3 w-3">
                <span
                  className={`animate-ping absolute inline-flex h-full w-full rounded-full ${area.dotColor} opacity-20`}
                ></span>
                <span
                  className={`relative inline-flex rounded-full h-3 w-3 ${area.dotColor}`}
                ></span>
              </div>
              ...
              <div>
                <span
                  className={`text-[11px] font-bold tracking-widest uppercase ${area.name === "Khu B" ? "text-orange-500" : "text-[#245A34]"}`}
                >
                  KHU VỰC
                </span>
              ...
              <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                ...
                {/* Status */}
                <div
                  className={`flex items-center font-bold text-[13px] ${area.statusColor}`}
                >
                  {area.statusIcon}
