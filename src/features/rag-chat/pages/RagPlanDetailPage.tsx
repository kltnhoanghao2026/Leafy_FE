...
import { getRagPlan } from "../api/ragChat.api";
...
export function RagPlanDetailPage() {
  ...
  const { data: detailData, isLoading, error } = useQuery({
    queryKey: ["ragPlan", planId],
    queryFn: () => getRagPlan(planId!),
    enabled: !!planId,
  });
  ...
  if (error || !detailData) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        ...
        <p className="text-slate-600 font-medium">KhÃÂ´ng thÃ¡Â»Æ tÃ¡ÂºÂ£i phÃÂ¡c ÃâÃ¡Â»â ÃâiÃ¡Â»Âu trÃ¡Â»â¹.</p>
        <button
          onClick={() => navigate(-1)}
  ...
  // according to RagPlanDoc structure, it's either in detailData.plan or detailData itself
  const plan = asPlanRecord(detailData.plan ? detailData.plan : detailData);

  const diseaseName = asPlanString(plan.diseaseName) || "PhÃÂ¡c ÃâÃ¡Â»â ÃâiÃ¡Â»Âu trÃ¡Â»â¹ chung";
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6 pb-20 animate-in fade-in duration-500">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl lg:text-3xl font-extrabold text-slate-900 tracking-tight">Chi tiÃ¡ÂºÂ¿t phÃÂ¡c ÃâÃ¡Â»â</h1>
      </div>
      ...
      <section className="bg-white rounded-3xl border border-slate-200 p-6 lg:p-8 shadow-xs">
            ...
            {typeof confidence === "number" && (
              <div className="mt-4 max-w-xs">
                <div className="flex items-center justify-between text-xs text-slate-500 mb-1.5">
                  <span>ÃÂÃ¡Â»â¢ tin cÃ¡ÂºÂ­y AI</span>
          ...
          <div className="mt-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider inline-flex items-center gap-2">
                ...
                LÃ¡Â»â¹ch trÃÂ¬nh sÃ¡Â»Â± kiÃ¡Â»â¡n ({schedule.length})
              </h3>
            ...
            {viewMode === "calendar" ? (
              <PlanCalendar schedule={schedule} />
            ) : (
          ...
          {requiredInputs.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-3">
                VÃ¡ÂºÂ­t tÃÂ° cÃ¡ÂºÂ§n thiÃ¡ÂºÂ¿t
              </h3>
        ...
        <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-6">
          <p className="text-sm text-slate-500">MÃÂ£ phÃÂ¡c ÃâÃ¡Â»â: <code className="bg-slate-100 px-1 py-0.5 rounded text-xs">{planId}</code></p>
          {planCost && (
            <p className="text-sm font-bold text-slate-700 bg-slate-50 px-3 py-1.5 border border-slate-200 rounded-lg">
              TÃ¡Â»â¢ng chi phÃÂ­ dÃ¡Â»Â± kiÃ¡ÂºÂ¿n: <span className="text-emerald-600">{planCost}</span>
            </p>
          )}
        </div>
      </section>
