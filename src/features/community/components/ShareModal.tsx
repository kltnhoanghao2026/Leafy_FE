...
import { X, Link2, Share2, RotateCcw, Check, MessageSquare } from 'lucide-react'
...
export function ShareModal({ isOpen, onClose, post }: ShareModalProps) {
  ...
  // Mock share link
  const shareLink = `${window.location.origin}/dashboard/community?post=${post.id}`
  ...
  const handleShareOptions = [
    ...
    },
    {
      ...
      action: () => {
        if (navigator.share) {
          navigator.share({ title: `Bài viết từ ${post.author.name}`, url: shareLink })
        } else {
          handleCopyLink()
        }
      }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-md mx-auto animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div className="w-8" />
          <h2 className="text-[17px] font-bold text-gray-900">Chia sẻ bài viết</h2>
          <button
            onClick={onClose}
        ...
        <div className="px-6 pt-5 pb-2">
          ...
          <div className="flex items-start gap-3 bg-slate-50 rounded-2xl p-4 border border-slate-200/80 mb-5">
            ...
            <div className="min-w-0">
              <p className="text-[13px] font-bold text-gray-900">{post.author.name}</p>
          ...
          <div className="mb-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[14px] font-bold text-gray-900">Chia sẻ lên trang của bạn</p>
            </div>
          ...
          <div className="relative my-2">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-3 text-[12px] font-bold text-slate-400 uppercase tracking-wider">Hoặc chia sẻ qua</span>
            </div>
          </div>
        </div>
          ...
          {handleShareOptions.map((option) => (
            ...
            >
              <div className="w-10 h-10 rounded-full bg-green-50 flex items-center justify-center shrink-0">
                <option.icon className="w-5 h-5" style={{ color: option.color }} strokeWidth={2.5} />
              </div>
              <span className="text-[15px] font-bold text-gray-900">{option.label}</span>
            </button>
          ))}
        </div>
        ...
        <div className="mx-6 mb-6 flex items-center gap-2 bg-slate-50 border border-slate-200/60 rounded-2xl px-4 py-2.5">
          ...
          <p className="text-[12px] text-slate-500 font-medium truncate flex-1">{shareLink}</p>
          <button
